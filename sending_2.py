...
def compare_times(time_str):
    try:
        time_to_compare = datetime.strptime(time_str, '%H:%M')
        current_time = datetime.now().time()

        time_diff = datetime.combine(datetime.today(), time_to_compare.time()) - datetime.combine(datetime.today(),
                                                                                                  current_time)
        if abs(time_diff.total_seconds() / 60) < 10:
            return True
        else:
            return False
    except ValueError:
        return "Invalid time format. Please use format 'HH:MM'."
...
'https://rnd.kassir.ru/frame/selection/3058?key=e039ce40-6380-cce1-e21a-4c920955737d&WIDGET_592250029=2fl1des5q5ljgtl06umcr9876n')
        ...
        if text=='рассылка' and compare_times('00:30'):
            send_message(user_id, 'Рассылка для подписчиков')
        elif text=='рассылка'and not (compare_times('00:30')) :
                send_message(user_id, 'Рассылки нет ')
